...
def import_and_create_bank(filename):
    ...
    for line in lines:
        ...
        #strip whitespace form beginning and end of line
        #split line into list based on ':' seperator
        ...
        #get key(name) and value (eposit amount) from line
        #strip whitespace from beginnignn of key (name) and value (deposit amount)
        key = lst[0].strip()
        value = lst[1].strip()
        ...
        try:
            ...
            #add new deposit amount to current toal balance
...
def valid_password(username, password):
    ...
    return(lower >=1 and upper >=1 and digit >= 1 and len(pword) >= 8 and (username !=password))
...
def import_and_create_accounts(filename):
    ...
    for line in lines:
        ...
        signup(user_accounts, log_in, ukey, pvalue)
...
def delete_account(user_accounts, log_in, bank, username, password):
    ...
    if (username in user_accounts) and (log_in.get(username)) and (user_accounts.get(username) == password):
